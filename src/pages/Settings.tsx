...
import { ArrowLeft, Key, Palette, CreditCard, BarChart3, Cpu } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-5xl mx-auto flex flex-col md:flex-row">
        {/* Tabs */}
        <div className="w-full md:w-[200px] border-b md:border-b-0 md:border-r border-border p-3 md:p-4 md:min-h-[calc(100vh-56px)] flex-shrink-0">
              ...
              <div className="space-y-8">
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between p-4 rounded-xl border border-border">
                    ...
                    <button onClick={toggleTheme} className="px-4 py-2 rounded-lg border border-border text-sm font-medium text-foreground hover:bg-surface-hover transition-colors capitalize">
                      {theme}
                    </button>
                  </div>
                  <div className="flex items-center justify-between p-4 rounded-xl border border-border">
                    ...
                    <select value={model} onChange={(e) => setModel(e.target.value)} className="px-3 py-2 rounded-lg border border-border text-sm bg-surface text-foreground outline-none">
                ...
                <div className="space-y-4">
                  {['OpenAI', 'Anthropic', 'Custom'].map((provider) => (
                    <div key={provider} className="p-4 rounded-xl border border-border">
                      ...
                      <input type="password" placeholder="sk-..." className="mt-2 w-full px-3 py-2 rounded-lg bg-surface border border-border text-sm text-foreground outline-none focus:ring-1 focus:ring-accent font-mono" />
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="space-y-8">
                ...
                <div className="p-6 rounded-xl border border-accent/30 bg-accent/5">
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    ...
                    <button className="px-4 py-2 rounded-lg border border-border text-sm font-medium text-foreground hover:bg-surface-hover transition-colors">Manage</button>
                  </div>
                </div>
              </div>
            )}
                ...
                <div className="p-4 rounded-xl border border-border">
                  ...
                  <div className="flex items-end gap-1.5 h-32">
                    {Array.from({ length: 14 }, (_, i) => {
                      ...
                      return (
                        <div key={i} className="flex-1 bg-accent/20 hover:bg-accent/40 rounded-t transition-colors cursor-pointer" style={{ height: `${height}%` }} />
                      );
                    })}
                  </div>
                  ...
                  {[
                    ...
                    <div key={m.name} className="flex items-center justify-between p-4 rounded-xl border border-border">
                      <div className="flex items-center gap-3">
                        <div className={cn("w-2 h-2 rounded-full", m.status === 'active' ? "bg-success" : "bg-muted-foreground/30")} />
                        <div>
                          <p className="text-sm font-medium text-foreground">{m.name}</p>
                          <p className="text-xs text-muted-foreground">{m.provider}</p>
                        </div>
                      </div>
                      <button className="px-3 py-1.5 rounded-lg border border-border text-xs font-medium text-muted-foreground hover:text-foreground hover:bg-surface-hover transition-colors">Configure</button>
                    </div>
                  ))}
                </div>
              </div>
